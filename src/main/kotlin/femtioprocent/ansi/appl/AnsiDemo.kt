        ...
        when {
            ...
            Am("d") -> {
                ...
                ((4..16) + listOf(256))
                    ...
                    .map { (cv, cv2) ->
                        println(
                            "$lightGreen".cvBg(lightGreen) + " -> " + "$cv".cvBg(cv) + " -> " + "$cv2".cvBg(
                                cv2
                            )
                        )
                    }
                ...
                ((4..16) + listOf(256))
                    ...
                    .map { (cv, cv2) ->
                        println(
                            "$cv256".cvBg(cv256) + " -> " + "$cv".cvBg(cv) + " -> " + "$cv2".cvBg(
                                cv2
                            )
                        )
                    }

                ((4..16) + listOf(256)).forEach {
                    val cv0 = Ansi.CubeValue(it, Random.nextInt(it - 1), Random.nextInt(it - 1), Random.nextInt(it - 1))
                    val cv1 = cv0.toCubeSize(256)
                    val cv2 = cv1.toCubeSize(it)
                    println("${"$cv0".cvBg(cv0)} -> ${"$cv1".cvBg(cv1)} -> ${"$cv2".cvBg(cv2)} ∆ ${cv0.r - cv2.r},${cv0.g - cv2.g},${cv0.b - cv2.b}")
                }
